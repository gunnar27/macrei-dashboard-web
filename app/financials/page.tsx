...
import { fetchFinancials, type PortfolioFinancials } from "@/lib/api";
      ...
      {error ? (
        ...
        <>
          ...
          <div className="chart-grid" style={{ marginBottom: 18 }}>
            ...
            <div className="panel">
              <div className="panel-title">Operating Expense by Category</div>
              <div className="panel-sub">Where the money goes (operating only)</div>
              {cats.length === 0 ? (
                <div className="dim" style={{ padding: "30px 0", fontSize: 13 }}>No operating expense in window.</div>
              ) : (
                  ...
                  {cats.slice(0, 10).map(([name, v]) => (
                    <div key={name}>
                      <div style={{ display: "flex", justifyContent: "space-between", fontSize: 13, marginBottom: 4 }}>
                        ...
                        <span className="num warn-c" style={{ fontWeight: 600 }}>{usd0(v)}</span>
                      </div>
                      <div className="meter"><span style={{ width: `${(v / maxCat) * 100}%`, background: "linear-gradient(90deg, #fbbf24, #f59e0b)" }} /></div>
                    </div>
                  ))}
                </div>
              )}
